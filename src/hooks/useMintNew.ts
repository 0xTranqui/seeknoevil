...
export function useMintNew({ mintNewConfig }: any) {
  ...
  const validMint =
    !mintingConfig || !mintNewConfig.tokenRendererInit.tokenURI ? false : true;
  ...
  const { config, error } = usePrepareContractWrite({
    ...
    // enabled: mintAccess,
    /*
     * @dev Hardcoded as zero for now but should be dynamic based on prior read call
     */
    // overrides: {}
  });
  ...
  console.log("Write error:", isError);

  const { data: mintNewData, isLoading: mintNewLoading } =
    useWaitForTransaction({
      hash: data?.hash,
