...
export function useMintWithData({ mintWithDataConfig, tokenToCurate }: any) {
  ...
  const ap721Press = process.env.NEXT_PUBLIC_AP_721_CURATION_CONTRACT
    ? process.env.NEXT_PUBLIC_AP_721_CURATION_CONTRACT
    : "";

    const ap1155Press = process.env.NEXT_PUBLIC_AP_1155_CONTRACT
    ? process.env.NEXT_PUBLIC_AP_1155_CONTRACT
    : "";
  ...
  const dataForMint: any = validMint
  ...
  const { config: curationConfig, error: curationError } =
    usePrepareContractWrite({
      address: ap721Press,
      abi: erc721Press_abi,
  ...
  const { data: mintWaitData, isLoading: mintWaitLoading } =
    useWaitForTransaction({
